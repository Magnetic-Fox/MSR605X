...
* `DEFAULT_RID` = b"\xff" (default, working Report ID for MSR605X)
...
#### Typical constants
* `ESC` = b"\x1b" (escape character code)
* `FS` = b"\x1c" (file separator character code)
...
* `START_SEQUENCE` = ESC + b"s" (data sequence start)
* `END_SEQUENCE_W` = b"?" + FS (data sequence end - for writing)
* `END_SEQUENCE` = END_SEQUENCE_W + ESC (data sequence end - helper for reading)
...
* `ISO_TRACK2_75BPI` = b"\x4b" (track 2 - 75bpi setting byte)
* `ISO_TRACK3_210BPI` = b"\xc1" (track 3 - 210bpi setting byte)
* `ISO_TRACK3_75BPI` = b"\xc0" (track 3 - 75bpi setting byte)

#### Command constants
* `CMD_RESET` = ESC + b"a" (reset command)
* `CMD_READ` = ESC + b"r" (ISO read command)
* `CMD_WRITE` = ESC + b"w" (ISO write command)
* `CMD_COMM_TEST` = ESC + b"e" (communication test command)
* `CMD_ALL_LED_OFF` = ESC + b"\x81" (all LEDs off command)
* `CMD_ALL_LED_ON` = ESC + b"\x82" (all LEDs on command)
* `CMD_GREEN_LED_ON` = ESC + b"\x83" (green LED on command)
* `CMD_YELLOW_LED_ON` = ESC + b"\x84" (yellow LED on command)
* `CMD_RED_LED_ON` = ESC + b"\x85" (red LED on command)
* `CMD_LEAD_ZERO_SET` = ESC + b"z" (leading zeroes setting command)
* `CMD_LEAD_ZERO_CHECK` = ESC + b"l" (leading zeroes gathering command)
* `CMD_ERASE_CARD` = ESC + b"c" (card erase command)
* `CMD_BPI_SELECT` = ESC + b"b" (BPI setting command)
* `CMD_READ_RAW` = ESC + b"m" (RAW read command)
* `CMD_WRITE_RAW` = ESC + b"n" (RAW write command)
* `CMD_GET_MODEL` = ESC + b"t" (device model gathering command)
* `CMD_GET_FW_VERSION` = ESC + b"v" (device firmware version gathering command)
* `CMD_BPC_SET` = ESC + b"o" (BPC setting command)
...
* `CMD_SENSOR_TEST` = ESC + b"\x86" (sensor test command)
* `CMD_RAM_TEST` = ESC + b"\x87" (RAM test command)
